...
selectMove b@(Board _ c) = 
  maximumBy (compare `on` (utility b)) $ possibleMoves
  where
...
utility' n b@(Board _ c) m  
  | null possibleMoves = 100
  | otherwise = naiveUtility b m - (maximum . map (utility' (n - 1) newb) . take 5 . reverse . sortBy (compare `on` naiveUtility newb) $ possibleMoves)
  where
    newb = (update b m)    
    possibleMoves = filter (not . inCheck (switchColor c) . update newb) . filter (legal newb) $ allMoves
